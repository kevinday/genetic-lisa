...
pub trait ShapeBehaviour {
    ...
    fn draw_onto(&self, _: &mut Canvas);
}
...
impl ShapeBehaviour for Rect {
    fn mutate(&mut self) {
        match (rand() * 100.) as u8 {
            0..=59 => self.color = self.color.mutate(),
            60..=69 => self.x = rand_adjust(self.x, 0.5, 0., 1.0),
            70..=79 => self.y = rand_adjust(self.y, 0.5, 0., 1.0),
            80..=89 => self.width = rand_adjust(self.width, 0.5, 0., 1.0),
            90..=99 => self.height = rand_adjust(self.height, 0.5, 0., 1.0),
            _ => panic!(),
        }
    }
...
impl ShapeBehaviour for Triangle {
    fn mutate(&mut self) {
        match (rand() * 100.) as u8 {
            0..=39 => self.color = self.color.mutate(),
            40..=49 => self.x1 = rand_adjust(self.x1, 0.5, 0., 1.0),
            50..=59 => self.y1 = rand_adjust(self.y1, 0.5, 0., 1.0),
            60..=69 => self.x2 = rand_adjust(self.x2, 0.5, 0., 1.0),
            70..=79 => self.y2 = rand_adjust(self.y2, 0.5, 0., 1.0),
            80..=89 => self.x3 = rand_adjust(self.x3, 0.5, 0., 1.0),
            90..=99 => self.y3 = rand_adjust(self.y3, 0.5, 0., 1.0),
            _ => panic!(),
        }
    }
...
impl ShapeBehaviour for Circle {
    fn mutate(&mut self) {
        match (rand() * 10.) as u8 {
            0..=3 => self.color = self.color.mutate(),
            4..=5 => self.x = rand_adjust(self.x, 0.5, 0., 1.0),
            6..=7 => self.y = rand_adjust(self.y, 0.5, 0., 1.0),
            8..=9 => self.rad = rand_adjust(self.rad, 0.5, 0.01, 1.0),
            10 => self.rad = rand_adjust(self.rad, 0.5, 0.01, 1.0), // handle the case for 10 explicitly
            _ => panic!()
        }
    }
